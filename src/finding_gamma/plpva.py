...
def plpva(x, xmin, *varargin):
    ...
    if Bt == []: # this indicates the number of repetitions, with 1000 being the default
        Bt = 10000
    ...
    if f_dattype== 'REAL':
        ...
        x = map(int, x)  # all this does is ensure all values in x are integers

        if vec == []:  # this will be true because we are using the default range, which we can change if we want
            for i in range(150, 351):
                vec.append(i/100.)    # covers range of most practical scaling parameters

        zvec = map(zeta, vec)
        z = filter (lambda X: X >= xmin, x)
        nz = len(z)  # nz represents the number of observations that exceed x_min
        y = filter (lambda X: X < xmin, x)
        ny = len(y)  # ny represents the number of observations that are below x_min
        xmax = max(z)  # so xmax is the largest observational datum
        L = []
