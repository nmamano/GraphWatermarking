...
def plfit(x, *varargin):
        ...
        if type(varargin[i]) == str:
            if varargin[i] == 'range':
                ...
                if Range[1] > Range[0]:
                    ...
                    vec = []
                try:
                    vec = map(lambda X:X*float(Range[2])+Range[0], range(int((Range[1]-Range[0])/Range[2])))
                except:
                    argok = 0
                    vec = []
    ...
    if f_dattype == 'REAL':
        # print("We are fitting to real numbers.")

        xmins = unique(x)
        xmins.sort()

        # at this point, we have eliminated any non-unique point and sorted the list xmins (which is our original x)
        # xmins contains the list of elements that we are interested in fitting

        xmins = xmins[0:-1]  # this step removes the largest value

        if xminx != []:  # this is only true if a value for alpha was passed into plfit
            xmins = [min(filter(lambda X: X >= xminx, xmins))]
        
        if limit != []: # this is only true if we want to exclude values for alpha greater than some limit
            xmins = filter(lambda X: X <= limit, xmins)
            if xmins == []:
                xmins = [min(x)]
            
        if sample != []: # this is only true if we want to use a different data set than the original
            step = float(len(xmins))/(sample - 1)
            index_curr = 0
            new_xmins = []
            for i in range (0, sample):
                if round(index_curr) == len(xmins):
                    index_curr -= 1
        ...
        dat = []
        z = sorted(x)  # all this does is create a sorted copy of the original data set

        # the following for loop finds the xmin value, the x-coordinate for which the power law holds after

        for i in range(0, len(xmins)):  # len(xmins) is equal to (len(original data set) - 1)
            xmin = xmins[i]
            ...
            if nosmall:  # this is only true if we call plfit with the 'nosmall' argument
                if (a - 1)/sqrt(n) > 0.1 and dat != []:
                    i = len(xmins) + 1
                    # I think there is a line of code missing here; Aaron Clauset's Matlab code shows the difference
                    break
        ...
        D = min(dat)
        xmin = xmins[dat.index(D)]  # index returns the smallest index for which D is in the array dat
        z  = filter(lambda X: X >= xmin, x)
        z.sort()
        n = len(z)
        alpha = 1 + n / sum(map(lambda X: log(float(X)/xmin), z))

        if finite:  # this is only true when we want a correction for a small data set
            alpha = alpha*float(n-1)/n+1.0/n
        ...
        # print("We are fitting to integers.")
        x = map(int, x)
        if vec == []:
            for X in range(150,351):
                vec.append(X/100.0)    # covers range of most practical
                                    # scaling parameters
        zvec = map(zeta, vec)
        ...
        for xm in range(0,len(xmins)):
            xmin = xmins[xm]
            # print 'This is the value of xmin: ', xmin
            z    = filter(lambda X: X >= xmin, z)
            n    = len(z)
